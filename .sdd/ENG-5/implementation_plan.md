...
**Completed 2026-05-09.** `solid_queue 1.x` installed alongside the existing `delayed_job_active_record` (DJ stays in tree for one-deploy rollback safety per plan; removed in step 8). Adapter switched: `config.active_job.queue_adapter = :delayed_job` → `:solid_queue` in `config/application.rb`.

**Single-DB twist not anticipated by the plan:** the SQ 1.x installer generates `db/queue_schema.rb` (a schema dump for a *separate* `queue` connection) instead of a migration in `db/migrate/`. Per locked decision (single DB), that file was discarded and replaced with a hand-rolled migration `db/migrate/20260509190000_create_solid_queue_tables.rb` containing all 11 tables (plan said "~9"; SQ 1.x ships 11 — adds `solid_queue_claimed_executions` and `solid_queue_recurring_tasks`). Migration ran cleanly on dev and test DBs. Smoke test: enqueued an AJ via `perform_later`, confirmed `solid_queue_jobs` and `solid_queue_ready_executions` populated.

**Operational changes:**
- `config/schedule.rb` — removed the 3-hour DJ cron restart entirely (Solid Queue is engineered to run as a long-lived process, not a periodically-restarted daemon). `whenever` still loaded; file is now empty of jobs.
- `config/deploy.rb` — removed `set :delayed_job_server_role`, `set :delayed_job_workers`. Added `after 'deploy:publishing', 'solid_queue:restart'` alongside the existing `deploy:restart`.
- `Capfile` — removed `require 'capistrano/delayed_job'` (the gem load).
- `Gemfile` — removed `gem 'capistrano3-delayed-job'`. `Gemfile.lock` clean of it. `delayed_job_active_record` and `daemons` kept for step 8.
- `lib/capistrano/tasks/solid_queue.rake` (new) — defines `solid_queue:start`, `:stop`, `:restart`, `:status`, all wrapping `sudo systemctl <action> bigtrees-solid-queue`. Auto-loaded by Capfile's `Dir.glob`. `cap -T` confirms the tasks are wired and the old `delayed_job:*` tasks are gone.
- `bin/jobs` (new, generated by installer) is present alongside `bin/delayed_job` (kept for rollback safety per plan).
- `config/queue.yml` (new): single dispatcher polling at 1s, single worker pool with 3 threads, processes from `JOB_CONCURRENCY` env (default 1). Reasonable for current zero-volume.
- `config/recurring.yml` (new): one production-only SQ housekeeping task (`SolidQueue::Job.clear_finished_in_batches` hourly). Not application code; SQ-internal.

**Remaining manual step (production server, NOT in this PR):** create `/etc/systemd/system/bigtrees-solid-queue.service` running `cd /var/www/bigtrees/current && bundle exec bin/jobs`, plus a sudoers entry granting the deploy user NOPASSWD on `systemctl {start,stop,restart,status} bigtrees-solid-queue`. Documented in step 7 of plan; flag prominently in PR description.

**Verification:** `bundle exec rspec` 165/0, zeitwerk clean, `bundle exec cap -T` lists `solid_queue:*` tasks (no `delayed_job:*`), end-to-end enqueue smoke test populates SQ tables correctly.
