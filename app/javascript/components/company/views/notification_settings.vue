<template>
  <div>
    <app-header title='Notification Preferences' />
...
export default {
  ...
  methods: {
    retrieveSettings() {
      this.axiosGet(`/organizations/${this.company.id}/notification_configurations`).then(response => {
    ...
    updateSetting(name, value) {
      this.axiosPut(`/organizations/${this.company.id}/notification_configurations/${name}`, { value: value })
    }
  },
  mounted() {
